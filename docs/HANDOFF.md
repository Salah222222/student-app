...
## MOB-P3 — إصلاح شات ANL-P3 على الموبايل + ربطه بالتنقل

الشات (`ChatPage.jsx`) كان اتسلّم في زيب منفصل (مشروع الـPHP backend)
ومش موجود في السنابشوت الأصلي بتاع `student-app` — اتنقل هنا. فحص الكود
الحقيقي (`AuthContext.jsx`, `apiClient.js`, `BottomNav.jsx`) طلّع باجين
حقيقيين في `useMessages.js` الأصلي (مش مجرد افتراض غير موثّق):

1. `useAuth()` الفعلي بيرجّع `{ user, isAuthenticated, login, logout }`
   بس — صفر `token`. `useMessages.js` الأصلي كان بيعمل
   `const { token } = useAuth()` → `undefined` دايمًا → صفر
   `Authorization` header → 401 دايمًا.
2. نفس الملف كان بيعمل `fetch("/api/v1/messages", ...)` بمسار نسبي —
   مخالف للقاعدة الموثّقة فوق ("كل استدعاء API URL كامل عبر
   `apiClient.js`"). من دومين Vercel هيدور على نفسه، مش على السيرفر.

**الحل:** `useMessages.js` اتكتب من جديد يستخدم `api.messages()`/
`api.sendMessage()`/`api.pollMessages()` (نفس نمط `useMyCourses.js`)،
بدل ما يدير الـheaders بنفسه. `ChatPage.jsx` نفسه صفر تعديل — نفس
الـshape الخارجي (`status`/`messages`/`sending`/`send`/`reload`).

**مدخل التنقل:** تاب خامس في `BottomNav.jsx` (مش زرار عائم، مش استبدال
تاب موجود) — زرار عائم هيكرر لبس موجود فعلًا عند المالك بين أيقونة
الواتساب القديمة والشات؛ استبدال "كل الكورسات" بيضحّي بقناة اكتشاف
أساسية. الأيقونة (`ChatCircleDots`) نفسها المستخدمة جوه `ChatPage.jsx`.
كل تاب `flex-1` أصلًا، فالتاب الخامس بيتوزّع تلقائي بلا حساب عرض جديد.

| الملف | الحالة |
|---|---|
| `src/lib/apiClient.js` | معدّل (3 methods جداد: `messages`/`sendMessage`/`pollMessages`) |
| `src/lib/useMessages.js` | استبدال كامل |
| `src/features/chat/ChatPage.jsx` | منقول من مشروع الـbackend، صفر تعديل |
| `src/App.jsx` | معدّل (route `/chat` محمي بـ`ProtectedRoute`) |
| `src/components/BottomNav.jsx` | معدّل (تاب خامس "الرسائل") |
| `docs/HANDOFF.md` | هذا القسم |

**Broken or incomplete:** نفس ملاحظة كل مرحلة قبل كده — **مفيش
`npm run build` ولا فتح حقيقي في متصفح** (نفس قيد الـsandbox الموثّق
فوق، صفر وصول لـnpm registry). كل تعديل اتفحص بمطابقة نمط `useMyCourses.js`/
`BottomNav.jsx` الأصلي حرفيًا، لكن لازم `npm install && npm run build`
أو نشر تجريبي على Vercel قبل التوثيق كـ"شغّال 100%". بادچ عدد الرسايل
غير المقروءة على تاب "الرسائل" لسه مش موجود — محتاج endpoint جديد في
`routes/api.php` (الـbackend) مش موجود حاليًا، برة نطاق المرحلة دي.
